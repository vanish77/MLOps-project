...
class TestDatasetStructure:
    ...
    def test_missing_split(self, sample_dataset):
        ...
        # Remove test split
        ds = DatasetDict({"train": sample_dataset["train"], "validation": sample_dataset["validation"]})
    ...
    def test_custom_splits(self, sample_dataset):
        ...
        assert validate_dataset_structure(sample_dataset, required_splits=["train", "validation"])
...
class TestDatasetBalance:
    ...
    def test_imbalanced_dataset(self):
        ...
        ds = DatasetDict(
            {
                "train": Dataset.from_dict({"text": ["text"] * 12, "label": [1] * 10 + [0] * 2}),
...
class TestTokenizedData:
    ...
    def test_missing_input_ids(self, sample_tokenized_data):
        ...
        ds = DatasetDict(
            {"train": Dataset.from_dict(data), "validation": Dataset.from_dict(data), "test": Dataset.from_dict(data)}
        )
    ...
    def test_exceeds_max_length(self):
        ...
        data = {"input_ids": [[1] * 600], "attention_mask": [[1] * 600], "label": [0]}  # Too long
