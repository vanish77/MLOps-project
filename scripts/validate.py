...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validate trained IMDb sentiment model on examples.")
    parser.add_argument("--model-path", required=True, help="Path to the trained model directory")
    parser.add_argument(
        "--examples", nargs="+", default=None, help="Custom examples to test (if not provided, uses default examples)"
    )
